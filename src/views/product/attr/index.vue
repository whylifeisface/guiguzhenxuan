...
const toLook = (row, $index) => {
  ...
  if (row.attrName.trim() == "")
    attrParam.attrValueList.slice($index,1);
  //过滤条件二 重复值拒绝添加
  attrParam.attrValueList.find((value) => {
    if (value == row.attrName){
      ElMessage.warning(`${value}已经存在`)
      attrParam.attrValueList.slice($index,1);
    }
  })
}
        ...
        <el-table border style="margin: 10px 0" :data="attrParam.attrValueList">
          ...
          <el-table-column label="属性名称">
            ...
            <template #default="{ row, $index }">
              ...
              />
              <div @click="toEdit(row)">{{ row.valueName }}</div>
